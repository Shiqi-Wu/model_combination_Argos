...
class EncoderDecoder(nn.Module):
    def __init__(self, params, emb1, emb2, encoder, decoder, generator):
    ...
    def forward(self, x1, u1, u2):
        src_mask, encoder_out = self.encode(x1, u1)
        #mu_en, sigma_en = self.generator(encoder_out)
        decoder_out = self.decode(encoder_out, u2, src_mask)
    ...
    def encode(self, x, u):
        ...
        x_u_cat = torch.cat((x, u), dim=-1)
        embeded = self.emb1(x_u_cat)
        encoder_out = self.encoder(embeded, None)
        
        return src_mask, encoder_out

    def decode(self, memory, u, src_mask):
        # tgt_mask = make_std_mask(x[:,:,0], 0)
        embeded = self.emb2(u)
        decoder_out = self.decoder(embeded, memory, None, None)
        return decoder_out
...
class Embedding(nn.Module):
    ...
    def forward(self, x, u):
        ...
        '''
        # onehot_embed = self.embedding(u) #[bs, windows_len, embedding_dim(default 20)] 
        try:
            output = torch.cat((x, u), dim=-1)
            output = self.embed1(output)
        except:
            embed()
        return output
